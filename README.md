    ...
    Order 
{
    ...
    total:number;   // содержит общую сумму покупки
    items:string[];  // содежрит id купленных товаров
...
interface IFormState {
 ...
 class Form<T> extends Component<IFormState> {
    ...
    //функция для генерации события 'change' изменения input-а формы, в котором передается название 
...
class OrderDeliveryView extends Form<IDelivery> {
    ...
    // конструктор принимает контейнер и экземляр  экземпляр EventEmitter
	constructor(container: HTMLFormElement, events: IEvents) 

	// сеттер для адреса
    set address(value: string) 

    // метод для генерации события изменения input 
    protected onInputChange(field: keyof IDelivery, value: string) 
...
// метод для генерации события изменения input 
    protected onInputChange(field: keyof IContact, value: string) 
	
}

-------------------------------------------------------
Класс для отображения модального окна удачной покупки  наследует класс Form параметризованный IOrderResult
